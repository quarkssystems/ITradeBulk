    ...
    <table class="table ">
        ...
        @foreach ($data as $datum)
            <tr>
                ...
                {{--<td>{{$datum->status}}</td>--}}
                <td>
                    <a class="btn btn-outline-primary" href="{{ route("$route.edit", $datum->uuid) }}" title="{{__("Edit")}}"><i class="fas fa-edit">{{__("Edit")}}</i></a>
                    {{--<a class="btn btn-outline-danger delete-item" href="javascript:;" data-form-id=".delete-form-{{$datum->uuid}}" title="{{__("Delete")}}"><i class="fas fa-trash"></i></a>
                    {!! Form::open(['route' => ["$route.destroy", $datum->uuid], 'method' => 'DELETE', 'class' => 'delete-form-'.$datum->uuid]) !!}
                    {!! Form::close() !!}--}}
                </td>
            </tr>
