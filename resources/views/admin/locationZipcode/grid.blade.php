                ...
                <td>
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                            <a class="dropdown-item" href="{{ route("$route.edit", [$datum->city_id, $datum->uuid]) }}" title="{{__("Edit")}}">{{__("Edit")}}</a>
                            <a class="dropdown-item delete-item" href="javascript:;" data-form-id=".delete-form-{{$datum->uuid}}" title="{{__("Delete")}}">{{__("Delete")}}</a>
                            <a class="dropdown-item" href="{{ route("$route.edit", [$datum->city_id, $datum->uuid]) }}?copy" title="{{__("Copy")}}">{{__("Copy")}}</a>
                        </div>
                    </div>
                    {!! Form::open(['route' => ["$route.destroy", $datum->city_id, $datum->uuid], 'method' => 'DELETE', 'class' => 'delete-form-'.$datum->uuid]) !!}
                    {!! Form::close() !!}
                </td>
            </tr>
        @endforeach
