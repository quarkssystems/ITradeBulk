...
class AdminProductImport implements ToModel, WithHeadingRow
{
  /**
   * @param array $row
   *
    ...
    if (isset($barcode) && !empty($barcode)) {
      ...
      if ($checkAvailableorNot) {
        ...
        if (isset($productBrand) && !empty($productBrand)) {
          $checkbrand = Brand::whereRaw('LOWER(`name`) = ? ', [strtolower($productBrand)])->first();
          if ($checkbrand === null) {
            $data = Brand::create(['name' => $productBrand, 'slug' => $productBrand]);
            $brandid = $data->uuid;
          } else {
        ...
        if ($checkproduct === null) {
          ...
          if ($promotion_id && $promotion_type && $period_from && $period_to && $promotion_price) {
            $promotionReqData = [
              ...
              'current_price' => $price,
              'status' => 'active',
              'user_id' => (string)$productModel->user_id,
          ...
          $inventoryReqData = [
            ...
            'single' => $quantity,
            'min_order_quantity' => $min_order_quantity,
            'stock_expiry_date' => ($stock_expiry_date != null) ? $stock_expiry_date : '',
            // 'audited' => $audited,
          ...
          $this->product_update++;
          $productModel = $productobj->withoutGlobalScopes()->where('barcode', $barcode)->update($arrProductRequest);
