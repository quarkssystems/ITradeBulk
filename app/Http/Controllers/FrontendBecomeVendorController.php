...
use App\Http\Requests\FrontendBecomeVendorRequest;
...
class FrontendBecomeVendorController extends Controller

{

    public $userRole = 'VENDOR';

    public $route = 'become-vendor';
...

    {
        ...
        $pageTitle = "Become Trader";

        $breadcrumb = [

            'home' => 'Home',

            'become-vendor' => $pageTitle

        ];

        return view('frontend.becomeVendor.form', compact('user', 'gender', 'title', 'type', 'roles', 'status', 'pageTitle', 'route', 'role', 'breadcrumb', 'businessType', 'countries', 'states', 'cities', 'zipcodes', 'foundingYears', 'documentTypes', 'banner'));

    }



    public function store(FrontendBecomeVendorRequest $request, User $user)

    {
        ...
        $company = array_merge($company, ['zipcode_id' => $request->get('zipcode_id'),
        ...
        //pending KYC notification

        

        return redirect('become-vendor')->with(['status' => 'success', 'message' => trans('success.frontend|become-vendor-request|success')]);
