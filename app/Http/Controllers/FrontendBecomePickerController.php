...
class FrontendBecomePickerController extends Controller
{
    public $userRole = 'PICKER';
    public $route = 'become-picker';
    ...
    public function index(Request $request, User $user, UserCompany $userCompanyModel, LocationCountry $locationCountryModel, LocationState $locationStateModel, LocationCity $locationCityModel, UserDocument $userDocumentModel) : View
    {
        ...
        $route = $this->route;
        $role = $this->userRole;
        $pageTitle = "Become Picker";
        $breadcrumb = [
            'home' => 'Home',
            'become-picker' => $pageTitle
        ];
        return view('frontend.becomePicker.form', compact('user', 'gender', 'title', 'type', 'roles', 'status', 'pageTitle', 'route', 'role', 'breadcrumb', 'businessType', 'countries', 'states', 'cities', 'zipcodes', 'foundingYears', 'documentTypes', 'banner'));
    }
        ...
        $email = EmailTemplate::where('name','=','supplier_KYC_pending_notification')->first();
            ...
            Mail::send([], [], function ($message) use ($userEmail , $emailContent) {
              $message->to($userEmail)
                ->subject('Picker - KYC Pending Notification')
                ->setBody($emailContent, 'text/html'); // for HTML rich messages
            });

        return redirect('become-picker')->with(['status' => 'success', 'message' => trans('success.frontend|become-supplier-request|success')]);
    }
}