...
class VarientsController extends Controller
{
    ...
    public $dataUrl = '/admin/varients';

    public $route = 'admin.varients';
    ...
    public function index(Request $request, Product $productModel, Excel $excel, Brand $brandModel, User $userModel)
    {
        ...
        $filters[] = [
            ...
            'column' => 'colour_variants',
            'operator' => 'LIKE',
            'sorting' => true,
            'search' => [
                'type' => 'text',
                'placeholder' => 'Search colour variants'
            ]
        ];

        $filters[] = [
            'title' => 'Size Variants',
            'column' => 'size_variants',
            'operator' => 'LIKE',
            'sorting' => true,
            'search' => [
                'type' => 'text',
                'placeholder' => 'Search size variants'
            ]
        ];
        ...
        if ($request->ajax()) {
            return view('admin.varients.grid', compact('data', 'dataGridTitle', 'dataGridSearch', 'dataGridPagination', 'route'));
        } else {
            return view('admin.varients.index', compact('data', 'dataGridTitle', 'dataGridSearch', 'dataGridPagination', 'pageTitle', 'route'));
        }
    }
    ...
    public function edit($id, Brand $brandModel, Tax $taxModel, Category $categoryModel, ProductUnit $productUnitModel)
    {
        ...
        $pageTitle = "EDIT PRODUCT VARIENTS";
        $route = $this->route;
        $role = auth()->user()->role;
        return view('admin.varients.form', compact('product', 'pageTitle', 'route'));
    }
